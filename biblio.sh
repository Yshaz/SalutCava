...
function emprunter {
    [[ $# < 5 ]] && erreur "Nombre incorrect d'arguments"
    
    [[ -f $1 ]] || erreur "$1 existe pas"

    [[ `grep -Ec ".*%.*%$4%.*%" "$1"` != 0 ]] && erreur "livre avec meme titre deja emprunte"
...
function emprunteur {
    ...
    [[ `grep -Ec "$2" "$1"` > 0 ]] || erreur "Aucun livre emprunte avec le titre '$2'"
...
function rapporter {
    ...
    [[ `grep -Ec ".*%.*%$2%.*%" $1` > 0 ]] || erreur "Aucun livre avec titre $2"
